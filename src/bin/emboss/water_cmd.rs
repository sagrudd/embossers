...
/// Options for the `water` subcommand.
#[derive(Debug, Args)]
pub struct WaterCmd {
    ...
    #[arg(long, default_value="water.txt")]
    pub outfile: PathBuf,
}
...
pub fn run(cmd: WaterCmd) -> Result<()> {
    ...
    let params = WaterParams{ matrix, gap_open: cmd.gapopen, gap_extend: cmd.gapextend, ..Default::default() };

    let aln = water(&a.seq, &b.seq, &params)?;

    // Pretty output
    let mut f = std::fs::File::create(&cmd.outfile).with_context(|| format!("create {}", cmd.outfile.display()))?;
    use std::io::Write;
    writeln!(f, "# EMBOSS-like WATER (Rust) result")?;
