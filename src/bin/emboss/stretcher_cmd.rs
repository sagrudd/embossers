//! CLI subcommand implementation. Use via `emboss stretcher`.
use std::fs::File;
use std::io::Read;
...
/// Options for the `stretcher` subcommand.
#[derive(Debug, Args)]
pub struct StretcherCmd {
    ...
    pub gapextend: f32,
    /// Threshold `n*m` at/below which a full DP is used for speed.
    #[arg(long, default_value_t=256)]
    pub fallback_threshold: usize,
    /// Output file for a human-readable alignment.
    #[arg(long, default_value="stretcher.txt")]
    pub outfile: PathBuf,
}
...
pub fn run(cmd: StretcherCmd) -> Result<()> {
    ...
    let params = StretcherParams{
        matrix,
        gap_open: cmd.gapopen,
        gap_extend: cmd.gapextend,
        scale: 2.0,
        fallback_threshold: cmd.fallback_threshold,
    };

    let aln = stretcher(&a.seq, &b.seq, &params)?;

    // Pretty output
    let mut f = std::fs::File::create(&cmd.outfile).with_context(|| format!("create {}", cmd.outfile.display()))?;
    use std::io::Write;
    writeln!(f, "# EMBOSS-like STRETCHER (Rust) result")?;
